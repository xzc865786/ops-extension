...
<template>
  <div v-if="ticket" class="space-y-4">
    <div class="card p-4">
      ...
      <p class="text-sm muted">{{ ticket.ticket_no }} · 创建者 {{ ticket.creator_user_id }}</p>
      <div class="whitespace-pre-wrap text-sm mt-2">{{ ticket.description }}</div>
      <div class="flex flex-wrap gap-2 mt-3 text-sm">
        <button class="btn-secondary btn-sm" @click="claim">认领</button>
        <button class="btn-secondary btn-sm" @click="unclaim">释放</button>
        <button class="btn-secondary btn-sm" @click="takeover">接管</button>
        <button class="btn-secondary btn-sm text-red-400" @click="close">关闭</button>
      </div>
      <div class="grid grid-cols-3 gap-2 mt-3 text-sm">
        <select v-model="patch.status" class="input">
          <option v-for="s in meta.statuses" :key="s.value" :value="s.value">{{ s.label }}</option>
        </select>
        <select v-model="patch.category" class="input">
          <option v-for="c in meta.categories" :key="c.value" :value="c.value">{{ c.label }}</option>
        </select>
        <select v-model="patch.priority" class="input">
          <option v-for="p in meta.priorities" :key="p.value" :value="p.value">{{ p.label }}</option>
        </select>
      </div>
      <button class="mt-2 btn-primary px-3 py-1 rounded text-sm" @click="savePatch">保存变更</button>
    </div>

    <div class="card p-4">
      <h2 class="font-medium mb-2">消息 / 内部备注</h2>
      <div v-for="m in ticket.messages" :key="m.id" class="border-b py-2 text-sm" :class="m.is_internal ? 'bg-amber-500/10' : ''">
      ...
      <button class="mt-2 btn-primary px-3 py-1.5 rounded text-sm" @click="send">发送</button>
    </div>
