  ...
  <div v-if="claim" class="space-y-4">
    ...
    <div class="card p-4 text-sm space-y-2">
      ...
      <div class="flex gap-2">
        <input v-model.number="payment.amount" type="number" class="input" />
        <input v-model="payment.reference_no" class="input" placeholder="流水号" />
        <button class="btn-primary px-3 py-1 rounded" @click="addPayment">付清并标记 PAID</button>
      </div>
    </div>
    ...
    <div class="card p-4 text-sm space-y-2">
      ...
      <div class="grid grid-cols-2 gap-2">
        <select v-model="invoice.invoice_status" class="input">
          <option>NONE</option><option>PENDING</option><option>RECEIVED</option><option>NOT_REQUIRED</option>
        </select>
        <select v-model="invoice.invoice_type" class="input">
          <option>VAT_SPECIAL</option><option>VAT_NORMAL</option><option>RECEIPT</option><option>OTHER</option>
        </select>
        <input v-model="invoice.invoice_number" class="input" placeholder="发票号" />
        <input v-model.number="invoice.tax_amount" type="number" class="input" placeholder="税额" />
      </div>
